 ...
 public:
  ...
  static std::vector<std::shared_ptr<Worker>> create_workers(
    ...
    if (context.protocol == "Serial") {
      ...
      // manager add a map to store lock

      std::vector<SparkleExecutor<WorkloadType> *> all_executors;

      // create worker
      for (auto i = 0u; i < context.worker_num; i++) {
        auto w = std::make_shared<SparkleExecutor<WorkloadType>>(
            coordinator_id, i, db, context, manager->transactions,
            manager->storages, manager->epoch, manager->lock_manager_status,
            manager->worker_status, manager->total_abort,
            manager->n_completed_workers, manager->n_started_workers, stop_flag,
            manager->NEXT_TX, manager->spec_scheduler_vec);
        workers.push_back(w);
        all_executors.push_back(w.get());
      }

      // create commit_threads
      // for (auto i = 0u; i < context.commit_threads; i++) {
      //   auto w = std::make_shared<SparkleExecutor<WorkloadType>>(
      //       coordinator_id, context.worker_num + i, db, context,
      //       manager->transactions, manager->storages, manager->epoch,
      //       manager->lock_manager_status, manager->worker_status,
      //       manager->total_abort, manager->n_completed_workers,
      //       manager->n_started_workers, stop_flag, manager->NEXT_TX,
      //       manager->spec_scheduler_vec);
      //   workers.push_back(w);
      //   all_executors.push_back(w.get());
      // }
      ...
      for (auto i = 0u; i < context.worker_num; i++) {
        static_cast<SparkleExecutor<WorkloadType> *>(workers[i].get())
            ->set_all_executors(all_executors);
      }

      // set commit_threads
      // for (auto i = 0u; i < context.commit_threads; i++) {
      //   static_cast<SparkleExecutor<WorkloadType> *>(workers[i +
      //   context.worker_num].get())
      //       ->set_all_executors(all_executors);
      // }
      ...
      std::vector<SpectrumExecutor<WorkloadType> *> all_executors;

      // create worker
      for (auto i = 0u; i < context.worker_num; i++) {
        auto w = std::make_shared<SpectrumExecutor<WorkloadType>>(
            coordinator_id, i, db, context, manager->transactions,
            manager->storages, manager->epoch, manager->lock_manager_status,
            manager->worker_status, manager->total_abort,
            manager->n_completed_workers, manager->n_started_workers, stop_flag,
      ...
      for (auto i = 0u; i < context.worker_num; i++) {
        static_cast<SpectrumExecutor<WorkloadType> *>(workers[i].get())
            ->set_all_executors(all_executors);
      }
    } else {
