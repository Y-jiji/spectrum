...
struct EVMQuery {
  ...
  // std::string Y_KEY[N];
  // bool UPDATE[N];
  std::vector<int32_t> pre_RKEY{};
  std::vector<int32_t> pre_WKEY{};
};
  ...
  void make_global_key_space_query(EVMQuery<N> &query, Context &context,
    ...
    switch (context.get_contract_type()) {
      ...
      case CONTRACT_TYPE::YCSB_10key_10r5w: {
        ...
        for (auto i = 0; i < vec.size(); i++) {
          query.INPUT_CODE += input_key_to_string(vec[i]);
          int offset = partitionID * context.keysPerPartition;
          if (context.sche_only_hotspots) {
            if (vec[i] <= context.pre_sched_num + offset) {
              if (i % 2 == 0) {
                query.WKEY.push_back(vec[i]);
              } else {
                query.RKEY.push_back(vec[i]);
              }
            } else if (vec[i] <= context.re_sched_num + offset) {
              if (i % 2 == 0) {
                query.pre_WKEY.push_back(vec[i]);
              } else {
                query.pre_RKEY.push_back(vec[i]);
              }
            }
          } else {
            if (i % 2 == 0) {
          ...
          // if (context.sche_only_hotspots) {
          //   if (vec[i] >= 200 + partitionID * context.keysPerPartition)
          //     continue;
          //   auto rand = random.uniform_dist(0, 99);
          //   if (rand < 50) {
          //     if (i % 2 == 0) {
          //       query.pre_WKEY.push_back(vec[i]);
          //     } else {
      ...
      case CONTRACT_TYPE::YCSB_10key_10r10w: {
        ...
        for (auto i = 0; i < vec.size(); i++) {
          ...
          // if (vec[i] >=
          //     context.pre_sched_num + partitionID * context.keysPerPartition)
          //     {
          //   continue;
          //   auto rand = random.uniform_dist(0, 99);
          //   if (rand < 50) {
      ...
      case CONTRACT_TYPE::SMALL_BANK_v2: {
        ...
        switch (opt) {
          case 0: {
            ...
            int offset = partitionID * context.keysPerPartition;
            if (context.sche_only_hotspots) {
              if (vec[0] <= context.pre_sched_num + offset) {
                query.WKEY.push_back(vec[0]);
              } else if (vec[0] <= context.re_sched_num + offset) {
                query.pre_WKEY.push_back(vec[0]);
              }
            } else {
              query.WKEY.push_back(vec[0]);
            }
          ...
          case 6: {
            ...
            if (context.sche_only_hotspots) {
              ...
              if (vec[1] <= context.pre_sched_num + offset) {
                query.WKEY.push_back(vec[1]);
              } else if (vec[1] <= context.re_sched_num + offset) {
                query.pre_WKEY.push_back(vec[1]);
              }
            } else {
              query.WKEY.push_back(vec[0]);
              query.WKEY.push_back(vec[1]);
