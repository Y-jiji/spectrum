...
class SparkleExecutor : public Worker {
 public:
  ...
  bool finalCommit(TransactionType &txn) {
    ...
    if (txn.id == NEXT_TX.load() && !txn.will_local_abort()) {
      NEXT_TX.fetch_add(1);
      // LOG(INFO) << "wr set size: " << txn.writeSet.size();
