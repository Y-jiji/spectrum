...
#include <dcc/protocol/Sparkle/SparkleHelper.h>
#include <dcc/protocol/Sparkle/SpecScheduler.h>
#include <unistd.h>
...
class SparkleExecutor : public Worker {
 public:
  ...
  using ProtocolType = Sparkle<DatabaseType>;
  ...
  SparkleExecutor(std::size_t coordinator_id, std::size_t id, DatabaseType &db,
                  ...
                  std::vector<std::unique_ptr<SpecScheduler>> &scheduler_vec)
      : Worker(coordinator_id, id),
        ...
        lock_manager_id(SparkleHelper::worker_id_to_lock_manager_id(
  ...
  void start() override {
    ...
    CHECK(n_lock_manager > 0)
        << "at least one sparkle_lock_manager";
    ...
    while (!stopFlag.load()) {
      if (!done_queue.empty()) {
        ...
        for (int j = 0; j < context.batch_size; ++j) {
          ...
          transaction->set_exec_type(ExecType::Exec_Sparkle);
  ...
  void set_all_executors(const std::vector<SparkleExecutor *> &executors) {
  ...
  void setupHandlers(TransactionType &txn) {
    txn.readRequestHandler = [this, &txn](SparkleRWKey &readKey,
                                          std::size_t tid,
                                          uint32_t key_offset) {
    ...
    txn.writeRequestHandler = [this, &txn](SparkleRWKey &writeKey,
                                           std::size_t tid,
                                           uint32_t key_offset) {
 ...
 private:
  ...
  std::vector<SparkleExecutor *> all_executors;
  ...
  std::vector<std::unique_ptr<SpecScheduler>> &spec_scheduler_vec;
