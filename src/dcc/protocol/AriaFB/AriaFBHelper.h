 ...
 public:
  ...
  static bool reserve_read(std::atomic<uint64_t> &a, uint64_t epoch,
    ...
    do {
      ...
      // LOG(INFO) << "old_epoch: " << old_epoch << "old_rts: " << old_rts;
  ...
  // the function first maps all partition id to 0, 1, 2 and then use % hash
  // to assign each partition to a lock manager.
