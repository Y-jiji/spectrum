 ...
 public:
  ...
  void search_for_read(std::size_t table_id, std::size_t partition_id,
    ...
    CHECK(tuple_num == writeSet.size() + readSet.size())
        << tuple_num << " " << writeSet.size() + readSet.size();
    // LOG(INFO) << id << " make checkpoint\n"
    ...
    // logical partition, physically one table for test
    // readKey.set_partition_id(partition_id);
    readKey.set_partition_id(0);
  ...
  void search_for_update(std::size_t table_id, std::size_t partition_id,
    ...
    // "\t" << "rdset len: " << this->readSet.size()  << "\n"
    // "\t" << "wrset len: " << this->writeSet.size() << std::endl;
    // localCheckpoint.push_back(std::make_tuple(key,
    // std::make_tuple(this->readSet.size(), this->writeSet.size())));
    CHECK(tuple_num == writeSet.size() + readSet.size());
    SpectrumRWKey readKey;
  ...
  bool will_local_abort() {
    // auto guard = std::lock_guard{rollback_key_mu};
    std::lock_guard<std::mutex> mu_lock(rollback_key_mu);
    return !rollback_key.empty();
  }

  void add_rollback_key(evmc::bytes32 key, int who) {
    // auto guard = std::lock_guard{rollback_key_mu};
    std::lock_guard<std::mutex> mu_lock(rollback_key_mu);
  ...
  virtual std::vector<int32_t> get_rdset() = 0;

  virtual std::vector<int32_t> get_wrset() = 0;

  virtual int get_wr_locked_idx() = 0;

  virtual void set_wr_locked_idx(int idx) = 0;

  virtual int get_rd_locked_idx() = 0;

  virtual void set_rd_locked_idx(int idx) = 0;

  virtual void update_rwset() = 0;
