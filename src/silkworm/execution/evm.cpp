...
class DelegatingTracer : public evmone::Tracer {
 public:
  explicit DelegatingTracer(EvmTracer &tracer,
                            IntraBlockState &intra_block_state) noexcept
 ...
 private:
  ...
  void on_instruction_start(uint32_t pc, const evmone::Stack &stack_top,
                            int stack_height,
                            const evmone::ExecutionState &state) noexcept {
    tracer_.on_instruction_start(pc, stack_top, stack_height, state,
                                 intra_block_state_);
  }

  void on_execution_end(const evmc_result &result) noexcept {
...
EVM::EVM(const Block &block, IntraBlockState &state,
...
CallResult EVM::execute(const Transaction &txn, uint64_t gas) noexcept {
  ...
  const evmc::address destination{contract_creation ? evmc::address{}
                                                    : *txn.to};
...
evmc::result EVM::create(const evmc_message &message) noexcept {
  ...
  if (nonce + 1 < nonce) {
    ...
    // See also
    // https://github.com/ethereum/go-ethereum/blob/v1.10.13/core/vm/evm.go#L426
  ...
  if (message.kind == EVMC_CREATE) {
    ...
    auto init_code_hash{
        ethash::keccak256(message.input_data, message.input_size)};
    contract_addr = create2_address(message.sender, message.create2_salt,
                                    init_code_hash.bytes);
  }

  state_.access_account(contract_addr);

  if (state_.get_nonce(contract_addr) != 0 ||
      state_.get_code_hash(contract_addr) != kEmptyHash) {
  ...
  res = evmc::result{execute(deploy_message,
                             ByteView{message.input_data, message.input_size},
                             /*code_hash=*/nullptr)};
  ...
  if (res.status_code == EVMC_SUCCESS) {
    ...
    } else if (res.gas_left >= 0 &&
               static_cast<uint64_t>(res.gas_left) >= code_deploy_gas) {
...
evmc::result EVM::call(const evmc_message &message) noexcept {
  ...
  if (message.kind != EVMC_DELEGATECALL &&
      state_.get_balance(message.sender) < value) {
    res.status_code = EVMC_INSUFFICIENT_BALANCE;
  ...
  if (is_precompiled(message.code_address)) {
    ...
    // const int64_t gas{static_cast<int64_t>(contract.gas(input.data(),
    // input.length(), revision()))}; if (gas < 0 || gas > message.gas) {
    //     res.status_code = EVMC_OUT_OF_GAS;
    // } else {
    ...
    if (code.empty() &&
        tracers_.empty()) {  // Do not skip execution if there are any tracers
      return evmc::result{res};
    }
...
evmc_result EVM::execute(const evmc_message &msg, ByteView code,
                         const evmc::bytes32 *code_hash) noexcept {
  const evmc_revision rev{revision()};

  // CHECK(!exo_evm) << "?????" << std::endl;
  // if (exo_evm) {
  //   // std::cout<<"extern interpreter"<<std::endl;
  //   EvmHost host{*this};
  //   return exo_evm->execute(exo_evm, &host.get_interface(), host.to_context(),
  //                           rev, &msg, code.data(), code.size());
  // if (code_hash && advanced_analysis_cache) {
  //   // std::cout<<"advanced interpreter"<<std::endl;
  ...
  // } else {
    ...
    // std::cout<<"baseline interpreter"<<std::endl;
...
gsl::owner<EvmoneExecutionState *> EVM::acquire_state() noexcept {
  if (execution_state == nullptr) {
    execution_state = new EvmoneExecutionState;
  }
  execution_state->id = id;
  return execution_state;
}

void EVM::release_state(gsl::owner<EvmoneExecutionState *> /*state*/) noexcept {}

evmc_result EVM::execute_with_baseline_interpreter(
    evmc_revision rev, const evmc_message &msg, ByteView code,
    const evmc::bytes32 *code_hash) noexcept {
  ...
  if (!analysis) {
    analysis = std::make_shared<evmone::baseline::CodeAnalysis>(
        evmone::baseline::analyze(code));
    if (use_cache) {
  ...
  if (state->will_partial_revert) {
    state->goto_checkpoint(state->partial_revert_key);
  } else {
    state->reset(msg, rev, host.get_interface(), host.to_context(), code);
  }

  const auto vm{static_cast<evmone::VM *>(evm1_)};
  // auto tx3_end_time{std::chrono::steady_clock::now()};

  // std::cout<<"internal 3:
  // "<<std::chrono::duration_cast<std::chrono::nanoseconds>(tx3_end_time-tx3_start_time).count()
  // <<" ns"<<std::endl;

  // auto tx4_start_time{std::chrono::steady_clock::now()};
  evmc_result res{evmone::baseline::execute(*vm, *state, *analysis)};

  // auto tx4_end_time{std::chrono::steady_clock::now()};
  // std::cout<<"internal 4:
  // "<<std::chrono::duration_cast<std::chrono::nanoseconds>(tx4_end_time-tx4_start_time).count()
  // <<" ns"<<std::endl;
...
// evmc_result EVM::execute_with_advanced_interpreter(
//     evmc_revision rev, const evmc_message &msg, ByteView code,
//     const evmc::bytes32 &code_hash) noexcept {
//   assert(advanced_analysis_cache != nullptr);

//   std::shared_ptr<evmone::advanced::AdvancedCodeAnalysis> analysis{
//       advanced_analysis_cache->get(code_hash, rev)};
//   if (!analysis) {
//     analysis = std::make_shared<evmone::advanced::AdvancedCodeAnalysis>(
//         evmone::advanced::analyze(rev, code));
...
evmc_revision EVM::revision() const noexcept {
  return config().revision(block_.header.number);
}
...
evmc_access_status EvmHost::access_account(
    const evmc::address &address) noexcept {
...
evmc_access_status EvmHost::access_storage(const evmc::address &address,
                                           const evmc::bytes32 &key) noexcept {
  return evm_.state().access_storage(address, key);
}

// for external database key partition
...
evmc::bytes32 EvmHost::get_storage(const evmc::address &address,
  ...
  // evmc::bytes32 read{evm_.state().get_current_storage(address, key)};
  // evmc::bytes32 read{evm_.state().get_storage_with_local(address, key,
  // false, evm_.txn_)};

  // HZC record read set
  // evm_.txn_->read_set[address][key] = read;
  // evmc::bytes32

  // first hit local then global
  auto tableId = 0;
  evmc::bytes32 read_val{};
  switch (evm_.txn_->wrapped_exec_type) {
    case dcc::ExecType::Exec_Serial: {
      auto wrapped_txn =
          static_cast<dcc::SerialTransaction *>(evm_.txn_->wrapped_txn);
    ...
    case dcc::ExecType::Exec_Spectrum: {
      auto wrapped_txn =
...
evmc_storage_status EvmHost::set_storage(
    const evmc::address &address, const evmc::bytes32 &key,
  ...
  switch (evm_.txn_->wrapped_exec_type) {
    case dcc::ExecType::Exec_Serial: {
      ...
      // if (!evm_.txn_->is_init) {
      //   evmc::bytes32 read_val{};
      //   wrapped_txn->search_for_update(
      //       tableId, get_partition_id(key, evm_.txn_->partition_num), key,
      //       read_val);
      //   // if (read == new_val) {
      //   //   // LOG(INFO)<<"HZC STORAGE_UNCHANGED: key:
      //   //   "<<silkworm::to_hex(key)<<", val:
      //   //   "<<silkworm::to_hex(read_val); return EVMC_STORAGE_UNCHANGED;
      //   // }
  ...
  // const evmc::bytes32 current_val{evm_.state().get_current_storage(address,
  // key)};
  ...
  // const evmc::bytes32
  // original_val{evm_.state().get_original_storage(address, key)};

  // // https://eips.ethereum.org/EIPS/eip-3529
  // const uint64_t sstore_clears_refund{rev >= EVMC_LONDON ? sstore_reset_gas
  // + fee::kAccessListStorageKeyCost
  //                                                        : fee::kRSClear};
...
evmc::uint256be EvmHost::get_balance(
...
evmc::bytes32 EvmHost::get_code_hash(
    const evmc::address &address) const noexcept {
...
size_t EvmHost::copy_code(const evmc::address &address, size_t code_offset,
                          uint8_t *buffer_data,
...
evmc::result EvmHost::call(const evmc_message &message) noexcept {
  if (message.kind == EVMC_CREATE || message.kind == EVMC_CREATE2) {
    ...
    if (res.status_code == EVMC_REVERT) {
      ...
      evmc::result res_with_no_output{res.status_code, res.gas_left, nullptr,
                                      0};
...
evmc_tx_context EvmHost::get_tx_context() const noexcept {
  ...
  const intx::uint256 effective_gas_price{
      evm_.txn_->effective_gas_price(base_fee_per_gas)};
  ...
  if (header.difficulty != 0) {
    ...
    std::memcpy(context.block_difficulty.bytes, header.mix_hash.bytes,
                kHashLength);
  }
  intx::be::store(context.chain_id.bytes,
                  intx::uint256{evm_.config().chain_id});
...
evmc::bytes32 EvmHost::get_block_hash(int64_t n) const noexcept {
  ...
  for (uint64_t i{old_size}; i < new_size; ++i) {
    std::optional<BlockHeader> header{
        evm_.state().db().read_header(base_number - i, hashes[i - 1])};
    if (!header) {
      break;
    }
...
void EvmHost::emit_log(const evmc::address &address, const uint8_t *data,
