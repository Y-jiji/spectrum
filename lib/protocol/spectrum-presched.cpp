...
void SpectrumPreSchedExecutor::Generate() {
    if (queue.Size() == 0) {
        ...
        for(auto& read: tx->GetReadSet()){
            if(read > 20) continue;
            auto belongs = read % queue_bundle.size();
            queue_bundle[belongs].Push(std::move(tx));
            flag = true;
            break;
        }
        
        // if(!flag){
        //     for(auto& write: tx->GetWriteSet()){
        //         if(write > 20) continue;
        //         auto belongs = write % queue_bundle.size();
        //         queue_bundle[belongs].Push(std::move(tx));
        //         flag = true;
        //         break;
        //     }
